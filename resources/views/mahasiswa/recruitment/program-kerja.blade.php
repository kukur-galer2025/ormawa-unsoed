...
<div x-data="{ modalOpen: false, modalTitle: '', modalImage: '', modalDesc: '' }">
...
@else
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div @click="modalOpen = true; modalTitle = '{{ addslashes($proker->nama) }}'; modalImage = '{{ $proker->foto ? Storage::url($proker->foto) : '' }}'; modalDesc = '{{ addslashes(str_replace(["\r", "\n"], ' ', $proker->deskripsi ?? '')) }}'"
             class="group bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-xl hover:shadow-blue-100/30 hover:-translate-y-1 transition-all duration-300 cursor-pointer">
            ...
            <div class="p-5">
                ...
                @if($proker->deskripsi)
                    <p class="text-sm text-slate-500 line-clamp-3 leading-relaxed">{{ $proker->deskripsi }}</p>
                @endif
