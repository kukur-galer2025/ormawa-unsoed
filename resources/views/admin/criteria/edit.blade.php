@extends("layouts.app")
@section("title","Edit Kriteria")@section("page-title","Edit Kriteria")
...
<form method="POST" action="{{ route('admin.criteria.update', [$recruitment, $criterion]) }}" class="space-y-5">@csrf @method('PUT')
...
<div>
    ...
    <input type="text" name="nama_kriteria" value="{{ old('nama_kriteria', $criterion->nama_kriteria) }}" required class="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
</div>
...
<div class="grid grid-cols-2 gap-4">
    <div>
        ...
        <select name="tipe" required class="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
            <option value="core" {{ old('tipe', $criterion->tipe)=='core'?'selected':'' }}>Core Factor (CF)</option>
            <option value="secondary" {{ old('tipe', $criterion->tipe)=='secondary'?'selected':'' }}>Secondary Factor (SF)</option>
        </select>
    </div>
    <div>
        <label class="block text-sm font-semibold text-slate-700 mb-1">Target (Nilai Ideal) *</label>
        <input type="number" name="target_value" value="{{ old('target_value', $criterion->target_value) }}" required min="1" max="5" class="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
    </div>
</div>

<div>
    <label class="block text-sm font-semibold text-slate-700 mb-1">Keterangan (Opsional)</label>
    <textarea name="keterangan" rows="2" class="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">{{ old('keterangan', $criterion->keterangan) }}</textarea>
</div>
...
<div class="border border-slate-200 rounded-xl p-4 bg-slate-50">
    ...
    <div class="space-y-2">
        @for($i = 1; $i <= 5; $i++)
        @php $existingLabel = $criterion->valueLabels->firstWhere('value', $i); @endphp
        <div class="flex items-center gap-3">
            ...
            <input type="text" name="labels[]" value="{{ old('labels.' . ($i-1), $existingLabel?->label ?? ['Sangat Kurang','Kurang','Cukup','Baik','Sangat Baik'][$i-1]) }}" required class="flex-1 px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
        </div>
        @endfor
    </div>
</div>
...
<div class="flex gap-3 pt-2">
    <button type="submit" class="px-6 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold rounded-xl shadow-lg shadow-blue-500/25">Update</button>
