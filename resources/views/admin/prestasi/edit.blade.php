...
<div class="w-full">
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
        <form method="POST" action="{{ route('admin.prestasi.update', $prestasi) }}" enctype="multipart/form-data" class="space-y-5">
            ...
            <div x-data="{ 
                previewUrl: '{{ $prestasi->foto ? asset('storage/' . $prestasi->foto) : '' }}', 
                errorMsg: '',
                fileChosen(event) {
                    ...
                    if (file) {
                        if (!file.type.startsWith('image/')) {
                            ...
                            this.previewUrl = '{{ $prestasi->foto ? asset('storage/' . $prestasi->foto) : '' }}';
                            event.target.value = '';
                            return;
                        }
                ...
                <label class="block text-sm font-bold text-slate-700 mb-2">Foto / Dokumentasi <span class="text-xs text-slate-400 font-normal">(opsional)</span></label>
