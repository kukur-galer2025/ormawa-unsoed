...
<div class="max-w-2xl bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
    <form action="{{ route('superadmin.jurusan.store') }}" method="POST" class="space-y-4">
        @csrf
        <div>
        ...
        <div>
            ...
            <input type="text" name="nama_jurusan" value="{{ old('nama_jurusan') }}" required class="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
            @error('nama_jurusan')<p class="mt-1 text-xs text-red-500">{{ $message }}</p>@enderror
        </div>
        <div class="pt-2">
            <button type="submit" class="px-6 py-2.5 bg-blue-600 text-white font-medium rounded-xl hover:bg-blue-700">Simpan Jurusan</button>
        </div>
    </form>
</div>
@endsection
